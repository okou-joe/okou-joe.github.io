<template>
  <div :class="$style.card">
    <nuxt-link :to="'/articles/'+ article.slug" :class="$style.card__container">
      <div :class="$style.card__thumbnail">
        <img v-if="article.thumbnail" :src="require(`~/assets/img/tmb/${article.thumbnail}`)" :alt="`${article.title}のサムネイル`">
...
.card {
  ...
  &__heading {
    ...
    font-weight  : normal;
    line-height  : 1;
    text-shadow  : 0 0 .5em rgba( c.$black, 1 );
    overflow     : hidden;
  ...
  &__thumbnail {
    ...
    &::after {
      ...
      background-color: rgba( c.$lightBlack, .6 );
